...
if(isset($_POST['cari'])){
  if ($jp=="notfound" || $cd=="") {
    ...
    echo "<script>document.location.href='/laundry/petugas'</script>";
  }else{
    $qw=$db->get("*","petugas","WHERE $jp LIKE '%$cd%'");
  }
...
?>
<!-- Judul Halaman -->
<title>Petugas | SIM Laundry</title>
<!--breadcrumbs start-->
  ...
  <div class="container">
    <div class="row">
      <div class="col s10 m6 l6">
        <h5 class="breadcrumbs-title">Petugas</h5>
  ...
  <div <?php echo $showcontent;?> class="section">
    <form action="petugas" method="POST">
      <select name="jenis_pencarian" class="kecilkecil">
        ...
        <option <?php if($jp=="kode_ptg"){echo "selected";}?> value="kode_ptg">Kode Petugas</option>
        <option <?php if($jp=="nama_ptg"){echo "selected";}?> value="nama_ptg">Nama Petugas</option>
        <option <?php if($jp=="Status"){echo "selected";}?> value="Status">Status</option>
      </select>
      <input required="" value="<?php echo $cd;?>" style="width: 20%;margin-left: 20px;margin-right: 20px;" type="text" name="cari_data" class="hide-on-med-and-downheader-search-input z-depth-2" placeholder="Masukkan Datamu Disini.." />
    ...
    <a href="input_petugas" class="waves-effect waves-light btn right"><i class="material-icons left">center_focus_weak</i> Tambah</a>
      ...
      <tbody>
        ...
        foreach($qw as $tamp_ptg){
          ...
          <tr>
            ...
            <td>
              <div class="kotak">
                <a href="index.php?p=input_petugas&kode_ptg=<?php echo $tamp_ptg['kode_ptg'];?>"><img src="images/icon/edit.png"></a>
                <a href="crud/d_petugas.php?kode_ptg=<?php echo $tamp_ptg['kode_ptg'];?>"><img src="images/icon/hapus.png"></a>
              </div>
            </td>
          </tr>
          <?php
